...
void Network<Arch, Transformer>::load(const std::string& rootDirectory, std::string evalfilePath) {
    ...
    for (const auto& directory : dirs)
    {
        if (evalFile.current != evalfilePath)
        {
			...
			if (directory == "<internal>" && (evalfilePath == "NNUE1" || evalfilePath == "NNUE2"))
			{
                load_ressource(evalfilePath);
			}
...
template<typename Arch, typename Transformer>
void Network<Arch, Transformer>::load_ressource(const std::string& resourceName) {
    try {
        // Lade die eingebettete Ressource als Stream
        auto stream = load_embedded_resource(resourceName);
        auto description = load(stream);

        if (description.has_value()) {
            evalFile.current = std::string(resourceName.begin(), resourceName.end()); // Wide-String nach String konvertieren
            evalFile.netDescription = description.value();
        }
    }
