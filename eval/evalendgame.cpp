...
value_t EvalEndgame::KBNK(MoveGenerator& position, value_t value) {
	...
	value = COLOR == WHITE ? MAX_BONUS + cornerValue - knightDistance : -MAX_BONUS + cornerValue + knightDistance;
	return value;
}
...
value_t EvalEndgame::forceToAnyCornerToMate(MoveGenerator& position, value_t value) {
	...
	value = COLOR == WHITE ? MAX_BONUS + distanceValue : -MAX_BONUS - distanceValue;
	return value;
}
